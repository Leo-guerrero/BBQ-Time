...
if (state == GameState.PAUSE) {
    ...
    var by1 = gh * 0.55;     // Restart button Y
    var by2 = by1 + 64;      // Menu button Y
    var bw  = 240, bh = 48;  // button size
    ...
    if (hover1 && mouse_check_button_pressed(mb_left)) {
        audio_stop_all();
        instance_activate_all();  // unfreeze world before changing rooms
        global.state = GameStates.RESTART;   // ensure not left paused
        room_restart();
    }
...
if (global.GameState == GameStates.GAMEOVER) {
	...
	// --- draw FinishRank_A centered above the Restart button ---
	// --- draw FinishRank_[Rank] centered above the Restart button (50% bigger) ---
	var spr_name = "FinishRank_" + global.Rank;                              // e.g., FinishRank_SPlus
